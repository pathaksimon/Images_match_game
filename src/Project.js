...
class Project extends Component {
  state = {
    ...
    newList: [
      ...
      },
      {
        ...
        imageUrl:
          'https://assets.ccbp.in/frontend/react-js/match-game/goa-img.png',
        thumbnailUrl:
      ...
      },
      {
        ...
        imageUrl:
          'https://assets.ccbp.in/frontend/react-js/match-game/fox-img.png',
        thumbnailUrl:
      ...
      },
      {
        ...
        imageUrl:
          'https://assets.ccbp.in/frontend/react-js/match-game/dog-img.png',
        thumbnailUrl:
  ...
  mani = clicked => {
    ...
    this.setState({scoring: 0})
    this.setState({timer: 60})
  }

  amazing = mid => {
    const {newList, id1, scoring, clicked} = this.state

    if (newList[0].id === mid) {
      this.setState({scoring: scoring + 1})
      const randy = newList.sort(() => Math.random() - 0.5)
      this.setState({newList: randy})
    } else {
  ...
  toggling = () => {
    const {categories, newList, scoring, clicked} = this.state

    const {first, second} = this.props
    const filtering = newList.filter(each => each.category === categories)
    const filtering1 = newList[0].imageUrl
    if (clicked) {
      return (
        <div>
          <div className="under-card1">
            <img src={filtering1} alt="match" className="firstPhoto" />
          </div>
          <div className="under-card2">
            <ul className="ul">
              {first.map(aich => (
                <Button
                  iterationofbitta={aich}
                  changing={this.settingup}
                  key={aich.tabId}
                />
              ))}
            </ul>
          </div>
          <div className="under-card3">
            <ul className="ull">
              {filtering.map(each => (
                <Images
                  iterationofimages={each}
                  key={each.id}
                  randomness={this.amazing}
                />
              ))}
            </ul>
          </div>
    ...
    return <Latest first1={this.state} mani={this.mani} />
  }
  ...
  timeup = () => {
    const {categories, newList, scoring, timer} = this.state
    if (timer <= 0) {
      return <Latest first1={this.state} mani={this.mani} />
    }
        ...
        <ul className="card1">
          <li className="firstone">
            <img
              ...
              alt="website logo"
              className="upper-left-photo"
            />
          </li>
          <li className="secondone">
            <p className="color">Score: {scoring}</p>
            <img
              src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
              alt="timer"
              className="upper-right-photo"
            />
            <p className="color">{this.timered()} Sec</p>
          </li>
        </ul>
        <div className="center-card">{this.timeup()} </div>
      </div>
    )
  }
}
